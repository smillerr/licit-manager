...
jest.mock("fs", () => {
  ...
  return {
    ...
    existsSync: jest.fn((path: string) => {
      // Simular que los archivos temporales existen
      return (
        path.includes("tmp") || path.includes("temp") || path.includes("test")
      );
    }),
  ...
  supabase: {
    storage: {
      from: jest.fn(() => ({
        upload: jest
          .fn()
          .mockResolvedValue({ data: { path: "test.pdf" }, error: null }),
        getPublicUrl: jest.fn(() => ({
          data: { publicUrl: "http://test.com/test.pdf" },
        })),
      })),
    },
  },
