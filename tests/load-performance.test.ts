...
describe("Pruebas de Carga y Rendimiento", () => {
  ...
  it("debe procesar múltiples PDFs concurrentemente", async () => {
    ...
    console.log(
      `⏱️  Tiempo total para ${concurrentRequests} requests: ${totalTime}ms`
    );
  ...
  it("debe manejar PDFs de diferentes tamaños eficientemente", async () => {
    ...
    for (const sizeInfo of sizes) {
      ...
      try {
        ...
        console.log(
          `✅ PDF ${sizeInfo.name} (${sizeInfo.size} bytes): ${processingTime}ms`
        );
      } catch (error) {
        results.push({
          size: sizeInfo.name,
  ...
  it("debe mantener estabilidad bajo carga prolongada", async () => {
    ...
    console.log(
      `🔄 Iniciando prueba de estabilidad: ${totalRequests} requests en ${duration}ms`
    );
